...
class GitAutoDeploy(object):
    ...
    def setup(self, config):
        ...
        if not self._config['allow-root-user'] and getpass.getuser() == 'root':
            logger.critical("Refusing to start as root. This application shouldn't run as a privileged user. Please run it as a different user. To disregard this warning and start anyway, set the config option \"allow-root-user\" to true, or use the command line argument --allow-root-user")
            sys.exit()
